...
function renderNzWizard() {
  ...
  const stepBar = `
    <div style="display:flex;align-items:center;gap:0;margin-bottom:22px">
      ${[{n:1,lbl:'Dane zlecenia'},{n:2,lbl:'Struktura G→P'}].map(({n,lbl}) => {
        const active = step === n;
        const done   = step > n;
        const col = done ? '#4ade80' : active ? '#3b82f6' : '#334155';
        const tcol = done||active ? '#fff' : '#475569';
        return `<div style="display:flex;align-items:center;flex:1">
          <div style="display:flex;align-items:center;gap:7px;flex:1">
            <div style="width:26px;height:26px;border-radius:50%;background:${col};color:#fff;
                        font-weight:700;font-size:.78rem;display:flex;align-items:center;justify-content:center;
                        flex-shrink:0">${done?'✓':n}</div>
            <span style="font-size:.8rem;font-weight:${active?700:400};color:${tcol}">${lbl}</span>
          </div>
          ${n<2?`<div style="height:2px;flex:1;background:${done?'#4ade80':'#1e293b'};margin:0 10px"></div>`:''}
        </div>`;
      }).join('')}
    </div>`;
  ...
  if (step === 1) {
    body = `
      ...
      <div class="field"><label>Nazwa zlecenia *</label>
        <input id="nz-nazwa" type="text" placeholder="np. Wał napędowy Ø50"
               value="${s.nzNazwa}"
               style="background:var(--panel);color:var(--text);border:1px solid var(--border);border-radius:6px;padding:8px 10px;width:100%;box-sizing:border-box;font-size:14px">
      </div>
      ...
      <div style="display:flex;gap:10px;margin-top:8px">
        ...
        <button onclick="nzGoStep2()"
                style="flex:2;background:#3b82f6;color:#fff;border:none;border-radius:6px;padding:10px;cursor:pointer;font-weight:700;font-size:.9rem">
          Dalej: Struktura G→P →
        </button>
      </div>`;
    ...
    function renderNzNode(node, depth) {
      ...
      const rowHtml = `
        <div style="margin-left:${indent}px;margin-bottom:3px;
                    background:${bg};border-left:2px solid ${borderCol};
    ...
    // Panel edycji prawej strony
    let editPanel = '';
    if (editNode) {
      if (editNode.typ === 'G') {
        const gOps = editNode.ops || [];
        const stanOptionsForOp = (stanowisko) => (state.stawki||[]).map(s =>
          `<option value="${s.stanowisko}" ${stanowisko===s.stanowisko?'selected':''}>${s.stanowisko}</option>`
        ).join('');
        ...
        const stanOptsP = (stanowisko) => (state.stawki||[]).map(s =>
          `<option value="${s.stanowisko}" ${stanowisko===s.stanowisko?'selected':''}>${s.stanowisko}</option>`
        ).join('');
          ...
          <div style="display:flex;flex-direction:column;gap:7px">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">
              <div>
                ...
                <input type="text" placeholder="np. P-001"
                       value="${editNode.symbol||''}"
              ...
              <div>
                ...
                <input type="text" placeholder="np. Wał Ø50"
                       value="${editNode.nazwa||''}"
        ...
        if (tryb === 'dlugosc') {
          trybFields = `
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px">
              <div>
                <label style="font-size:.72rem;color:#64748b;display:block;margin-bottom:3px">Długość na 1 szt (mm)</label>
                <input type="number" step="0.1" min="0"
                       value="${editNode.mat_dlugosc||0}"
                       onchange="nzUpdateField('${editNode._id}','mat_dlugosc',+this.value)"
                       style="background:#0f172a;color:#e2e8f0;border:1px solid #1e293b;border-radius:5px;padding:6px 8px;width:100%;box-sizing:border-box;font-size:.8rem">
              </div>
              <div>
                <label style="font-size:.72rem;color:#64748b;display:block;margin-bottom:3px">Ilość pręt./el. na wyrób</label>
                <input type="number" step="0.01" min="0.01"
                       value="${editNode.mat_ilosc_szt||1}"
          ...
          trybFields = `
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px">
              <div>
                <label style="font-size:.72rem;color:#64748b;display:block;margin-bottom:3px">Długość (mm)</label>
                <input type="number" step="0.1" min="0"
                       value="${editNode.mat_wys||0}"
                       onchange="nzUpdateField('${editNode._id}','mat_wys',+this.value)"
                       style="background:#0f172a;color:#e2e8f0;border:1px solid #1e293b;border-radius:5px;padding:6px 8px;width:100%;box-sizing:border-box;font-size:.8rem">
              </div>
              <div>
                <label style="font-size:.72rem;color:#64748b;display:block;margin-bottom:3px">Szerokość (mm)</label>
                <input type="number" step="0.1" min="0"
                       value="${editNode.mat_szer||0}"
                       onchange="nzUpdateField('${editNode._id}','mat_szer',+this.value)"
                       style="background:#0f172a;color:#e2e8f0;border:1px solid #1e293b;border-radius:5px;padding:6px 8px;width:100%;box-sizing:border-box;font-size:.8rem">
              </div>
              <div>
                <label style="font-size:.72rem;color:#64748b;display:block;margin-bottom:3px">Szt na wyrób</label>
                <input type="number" step="0.01" min="0.01"
                       value="${editNode.mat_ilosc_szt||1}"
                       onchange="nzUpdateField('${editNode._id}','mat_ilosc_szt',+this.value)"
                       style="background:#0f172a;color:#e2e8f0;border:1px solid #1e293b;border-radius:5px;padding:6px 8px;width:100%;box-sizing:border-box;font-size:.8rem">
          ...
          trybFields = `
            <div>
              <label style="font-size:.72rem;color:#64748b;display:block;margin-bottom:3px">Ilość sztuk na 1 wyrób</label>
              <input type="number" step="0.01" min="0.01"
                     value="${editNode.mat_ilosc_szt||1}"
                     onchange="nzUpdateField('${editNode._id}','mat_ilosc_szt',+this.value)"
                     style="background:#0f172a;color:#e2e8f0;border:1px solid #1e293b;border-radius:5px;padding:6px 8px;width:100%;box-sizing:border-box;font-size:.8rem">
              ...
              <div style="max-height:140px;overflow-y:auto;display:flex;flex-direction:column;gap:2px">
                ${matResults.map(m => `
                  <div onclick="nzSelectMat('${editNode._id}',${JSON.stringify(m).replace(/"/g,'&quot;')})"
                       style="padding:5px 8px;border-radius:4px;border:1px solid #1e293b;background:#0f172a;
    ...
    body = `
      <div style="display:flex;flex-direction:column;gap:14px;min-height:340px">
        <!-- Górna: drzewo -->
        <div style="display:flex;flex-direction:column;gap:8px">
          <div style="font-size:.72rem;font-weight:700;color:#475569;text-transform:uppercase;letter-spacing:.5px">
            Struktura drzewa
          </div>
          <div style="background:#0f172a;border:1px solid #1e293b;border-radius:8px;padding:10px;
                      overflow-y:auto;max-height:260px;min-height:80px;overflow-x:auto">
      ...
      <div style="display:flex;gap:10px;margin-top:14px">
        <button onclick="setState({nzStep:1})"
                style="background:#ffffff10;color:#94a3b8;border:1px solid #ffffff15;border-radius:6px;
                       padding:10px 16px;cursor:pointer;font-size:.85rem">
          ← Wróć
        </button>
        <button onclick="nzSave()" ${s.nzSaving?'disabled':''}
                style="flex:1;background:${s.nzSaving?'#1e293b':'#16a34a'};color:${s.nzSaving?'#475569':'#fff'};
                       border:none;border-radius:6px;padding:10px;cursor:${s.nzSaving?'wait':'pointer'};
